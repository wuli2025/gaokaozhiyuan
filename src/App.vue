...
// v3 PRD §03 6+1 网络化 Hub
import MajorHub from "./components/MajorHub.vue";
import CareerHub from "./components/CareerHub.vue";
import TruthHub from "./components/TruthHub.vue";
import RuleHub from "./components/RuleHub.vue";
...
function onSplashDone() {
  // 志愿产品：跳过母体的"选工作目录"引导，首屏直达智能填报。
  // 建档由 MatchRun 的输入面板承担（省份/分数/选科 → 位次）。
  ...
  <div class="shell" :style="{ gridTemplateColumns: layoutCols }">
    ...
    <main class="main">
      ...
      <KeepAlive :include="['KnowledgeGraph']">
        ...
        <MajorHub v-else-if="mountedView === 'major'" />
        <CareerHub v-else-if="mountedView === 'career'" />
        <TruthHub v-else-if="mountedView === 'truth'" />
        <RuleHub v-else-if="mountedView === 'rule'" />
        ...
        />
        <KnowledgeRules v-else-if="mountedView === 'knowledge'" />
    ...
    <!-- 启动流程覆盖层：splash → onboarding(v3: 3 步建画像) -->
