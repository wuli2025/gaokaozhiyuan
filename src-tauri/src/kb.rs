...
/// 上下文预算切分（PRD v2 §06 ③）
/// 总预算 24K 字符（远低于 32k，给模型回话留足）。
/// 预算分配：导航页 55% / 地图清单 40%；单页正文上限 30%。
/// 超出优雅截断，并提示"其余请用 Read/Glob 自取"。
pub const KB_CTX_BUDGET: usize = 24_000;
const NAV_PCT: usize = 55;
const NAV_SINGLE_CAP_PCT: usize = 30;

pub fn kb_context_block_scoped(query: &str) -> String {
    let root = KB_ROOT.read().clone();
    if root.as_os_str().is_empty() || !root.exists() {
        return String::new();
    }
    let hits = kb_search(query.to_string(), Some(8));
    if hits.is_empty() { return String::new(); }

    // 1. 预算切片
    let nav_budget = KB_CTX_BUDGET * NAV_PCT / 100;  // 13200
    let list_budget = KB_CTX_BUDGET - nav_budget;    // 10800
    let single_cap = KB_CTX_BUDGET * NAV_SINGLE_CAP_PCT / 100;  // 7200

    let mut out = String::from("\n\n## 维基库召回 (KB-first · 上下文预算 24K)\n\n");
    out.push_str(&format!("本块按 PRD v2 §06 切预算: 导航 {:.0}%, 清单 {:.0}%, 单页正文上限 {:.0}%。超出部分已优雅截断，其余请用 Read/Glob 自取。\n\n",
        NAV_PCT as f64, 100.0 - NAV_PCT as f64, NAV_SINGLE_CAP_PCT as f64));

    // 2. 导航：每条 hit 的 frontmatter + 第一段（标题+链接）
    out.push_str("### §1 导航(命中页面入口)\n\n");
    let mut nav_used = 0;
    for h in hits.iter() {
        let full = root.join(&h.path);
        let body = fs::read_to_string(&full).unwrap_or_default();
        let fm_end = body.find("\n---\n").unwrap_or(0);
        let _fm = if fm_end > 0 { &body[..fm_end] } else { "" };
        let h1_start = body.find("\n# ").map(|i| i + 1).unwrap_or(0);
        let h1_end = body[h1_start..].find("\n## ").map(|i| h1_start + i).unwrap_or(body.len());
        let first_para = body[h1_start..h1_end].chars().take(200).collect::<String>();
        let snippet = h.snippet.chars().take(80).collect::<String>();
        let block = format!("- **{}**  ({})\n  - path: `{}`  score: {:.1}\n  - 首段: {}\n",
            h.title, snippet, h.path, h.score,
            first_para.replace('\n', " ").trim());
        if nav_used + block.len() > nav_budget {
            out.push_str(&format!("\n_(§1 已用 {} / {} 字符, 后续 hit 略, 用 Read 自取)_\n", nav_used, nav_budget));
            break;
        }
        out.push_str(&block);
        nav_used += block.len();
    ...
    for (i, h) in hits.iter().enumerate() {
        ...
        let mut pos = 0;
        while let Some(h2_idx) = body[pos..].find("\n## ") {
            let abs = pos + h2_idx + 1;
            let next_h2 = body[abs + 4..].find("\n## ").map(|j| abs + 4 + j).unwrap_or(body.len());
            chunks.push(&body[abs..next_h2]);
            pos = next_h2;
        }
        if chunks.is_empty() { chunks.push(&body); }
        let mut entry = String::new();
        entry.push_str(&format!("\n---\n\n#### {}. **{}** (`{}`)\n", i + 1, h.title, h.path));
        let mut entry_used = 0;
        for ch in chunks.iter().take(6) {
            if entry_used + ch.len() > single_cap {
                let remain = single_cap.saturating_sub(entry_used);
                if remain > 100 && remain < ch.len() {
                    let safe = clamp_char_boundary(ch, remain);
                    entry.push_str(&ch[..safe]);
                    entry.push_str("\n_(本节截断, 其余 Read 自取)_\n");
                }
                break;
            }
        ...
        if list_used + entry.len() > list_budget {
            out.push_str(&format!("\n_(§2 已用 {} / {} 字符, 后续 hit 略)_\n", list_used, list_budget));
            break;
        }
        out.push_str(&entry);
        list_used += entry.len();
    ...
    // 4. 收尾
    out.push_str("\n### §3 用法提示\n\n");
    out.push_str("- 命中带 `gaps` 字段的页 → 标 `⚠ 资料不足` 后可继续追问\n");
    out.push_str("- 想要更详细 → 用 Read 工具打开对应 .md\n");
    out.push_str("- 想要沿双链走 → 用 Grep `\\[\\[(.*?)\\]\\]` 找链向\n");
    out
}
